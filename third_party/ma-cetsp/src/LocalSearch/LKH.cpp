...
void LKH::configure(std::string executable, std::string temporary_root) {
    lkh_exe = std::move(executable);
    root_dir = std::move(temporary_root);
}
...
void LKH::setContext(std::string timestamp, int random_value) {
    if (lkh_exe.empty()) {
        if (ENV == "LOCAL") {
            ...
            root_dir = SERVER_LKH_TMP_ROOT;
        }
    }
    if (lkh_exe.empty() || root_dir.empty()) {
        throw std::runtime_error("LKH executable and temporary root are required");
    }
    const std::filesystem::path run_root =
        std::filesystem::path(root_dir) /
        (timestamp + "_" + std::to_string(random_value));
    root_dir = run_root.string();
    params_file = (run_root / "params.par").string();
    problem_file = (run_root / "problem.tsp").string();
    tour_file = (run_root / "tour.txt").string();
    result_file = (run_root / "result.txt").string();
}
...
List* LKH::run(List* solution, bool adapted){
    ...
    write(solution);
#ifdef _WIN32
    std::string str = "\"\"" + lkh_exe + "\" \"" + params_file + "\" > NUL\"";
#else
    std::string str = "\"" + lkh_exe + "\" \"" + params_file + "\" > /dev/null";
#endif
    const char *cmd = str.c_str();
    const int exit_code = system(cmd);
    if (exit_code != 0) {
        throw std::runtime_error("LKH exited with code " + std::to_string(exit_code));
    }
...
List* LKH::read() {
    ...
    in.open(result_file);
    if (!in) {
        throw std::runtime_error("LKH did not produce a result tour");
    }
    ...
    while (data != "TOUR_SECTION") {
        if (!getline(in, data)) {
            throw std::runtime_error("invalid LKH result tour");
        }
    }
    getline(in, data);
    while (data != "-1") {
        ...
        solution->add(node);
        if (!getline(in, data)) {
            throw std::runtime_error("truncated LKH result tour");
        }
    }
    in.close();
    return solution;
