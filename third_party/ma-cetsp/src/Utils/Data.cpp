...
Data::Data(Parameters *params) {
    ...
    this->instance_file = params->instance_file;
    this->requested_result_file = params->result_file;

    if (!instance_file.empty()) {
        filename = std::filesystem::path(instance_file).stem().string();
        return;
    }
...
void Data::read() {
    ...
    if (!instance_file.empty()) {
        std::ifstream input(instance_file);
        if (!input) {
            throw std::runtime_error("could not open instance file: " + instance_file);
        }
        std::size_t count = 0;
        if (!(input >> count) || count == 0) {
            throw std::runtime_error("invalid instance header: " + instance_file);
        }
        centers.clear();
        ...
        for (std::size_t index = 0; index < count; ++index) {
            ...
            double y = 0.0;
            double radius = 0.0;
            if (!(input >> x >> y >> radius) || radius < 0.0) {
                throw std::runtime_error("invalid instance row: " + instance_file);
            }
            centers.push_back({x, y, radius});
        }
        return;
    }
    int file_type;
...
void Data::write(List* solution, int iter, std::string running_time) {
    try {
        if (!requested_result_file.empty()) {
            const std::filesystem::path output_path(requested_result_file);
            ...
            result_filename = output_path.string();
            Node* p = solution->head();
            for (int i = 0; i < solution->size(); ++i) {
                out << p->id << (i + 1 == solution->size() ? '\n' : ' ');
                p = p->next;
            }
            out << "value " << solution->getValue()
